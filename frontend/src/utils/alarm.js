...
export const alarmService = {
  ...
  removeAlarm: (id) => {
    ...
    // 삭제된 알람이 매칭 상태였다면, 상대방 알람도 매칭 해제
    if (alarmToRemove && alarmToRemove.matched) {
      alarmService.recalculateMatchings();
    }
  },

  // 매칭 상태 재계산 (알람 삭제 후 호출)
  recalculateMatchings: () => {
    const alarms = alarmService.getAllAlarms();
    
    const updatedAlarms = alarms.map(alarm => {
      // 현재 알람의 반대 조건 알람이 있는지 확인
      const reverseAlarm = alarms.find(
        a => a.id !== alarm.id && 
             a.myId === alarm.targetId && 
             a.targetId === alarm.myId
      );
      
      // 반대 알람이 있으면 매칭, 없으면 매칭 해제
      return { ...alarm, matched: !!reverseAlarm };
    });
