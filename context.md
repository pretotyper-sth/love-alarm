...
- **받은 메시지 탭**: 접근 시 → 인증 바텀시트 (메시지 기능 도입할 때 함께 도입)
- **더보기 탭 상단**: 메시지 기능 도입 시점부터 "인스타그램 연동" 항목 상단 노출 — 미리 인증해두려는 유저 자발적 진입 가능
...
- **메시지(선택) 필드**:
  - 상대 ID 아래 textarea 노출
  - 본인을 알 수 있는 힌트 또는 평소에 하고 싶었던 말을 남길 수 있음
  - 서로 연결되면 상대에게 공개됨
  - 보낸 메시지는 메시지함에서 다시 확인 가능
- **본인 ID 필드 동작**:
  - 미인증: 자유 입력 (현재 상태 유지)
  - 인증 완료 후: 인증된 ID 자동 채워짐 + 수정 불가 (읽기 전용)

### 8. 메시지 확인 페이지
- "내가 보낸 메시지" 탭: 미인증에서도 접근 가능
- "받은 메시지" 탭: 인증 필요 → 미인증 시 인증 CTA 노출
- 탭 전환 시 인증 CTA가 올바르게 표시/숨김 처리됨
- **메시지 상세 바텀시트**: 받은 메시지 행 탭 시 전체 내용 + 반응 선택 한 화면에서 처리
  - 리스트에서는 줄 생략(preview), 탭하면 시트에서 전체 내용 표시
  - 시트 하단 반응 이모지: ❤️ 😊 🤔 😳 🥹
  - 이미 반응한 이모지 하이라이트 표시
  - 개별 메시지를 열었을 때만 읽음 처리
- **신고 기능**:
  - 받은 메시지 상세 하단에서 신고 진입
  - 신고 사유 선택 후 제출하면 받은 목록에서 숨김
- 발신자 표기: "알 수 없는 누군가로부터" (회색, 12px)
- 알람 목록 상단 배지: 숫자 대신 red dot만 노출
- unread 기준: 메시지 페이지 진입이 아니라 개별 열람 기준
...
> 기준 정리: `main` 브랜치는 **검수 1 완료본**으로 간주한다. 현재 추가 검수 브랜치는 `review/2`부터 시작한다.
...
- [ ] **Unit 5**: 알람 추가 — 하루 추가 제한 (`총 슬롯 × 2`) + copy 수정

### 검수 2 — 인스타그램 인증 도입 ✅ 승인 완료 (2026-04-08)
> Unit 6 단독. Meta 앱 Live 전환 + Railway webhook URL 교체 완료.
> 메시지 기능(검수 4)과 함께 들어가므로, 검수 2는 인증 인프라 + 더보기 연동 항목이 핵심.
...
- [ ] **Unit 6**: 인증 바텀시트 전체 구현
  ...
  - 성공: "인증을 완료했어요" 토스트
  - **진입점**: 더보기 탭 상단 "인스타그램 연동" 항목 (자발적), 받은 메시지 탭 접근 시, 랭킹 노출 동의 토글 ON 시
...
### 검수 4 — 메시지 기능 end-to-end ✅ 승인 완료 (2026-04-14)
> Unit 8 단독. 서버 메시지 API + 인증 시스템 의존.

- [x] **Unit 8**: 메시지 페이지
  - 알람 추가 페이지: 메시지(선택) 입력 필드 복원
  - 탭 구조 (내가 보낸 메시지 / 받은 메시지)
  - 받은 메시지 탭: 미인증 시 인증 CTA
  - 메시지 상세 바텀시트 (전체 내용 + 이모지 반응: ❤️ 😊 🤔 😳 🥹)
  - 개별 열람 기반 unread 처리
  - 알람 목록 상단 💬 버튼 + red dot 배지
  - 신고하기 FE/BE 연결
...
- [ ] **Unit 9**: 알림 · 로깅 · 설정 개선
  - 메시지 수신 시 푸시/토스 앱 알림 발송 (스마트 메시지 A/B 템플릿 승인 완료)
  - 알림 토글에 설명 텍스트 추가 (각 토글이 무엇을 하는지 안내)
  - 이벤트 로깅: 전 페이지 screen/click 이벤트 수집 → AnalyticsEvent 테이블 저장
